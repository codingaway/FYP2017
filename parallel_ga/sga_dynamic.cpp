...
void copy_gene(struct genotype& copyFrom, struct genotype& copyTo, int geneCount);
...
void copy_gene(struct genotype &copyFrom, struct genotype &copyTo,
        int geneCount){
...
//    population[POPSIZE].fitness = population[best_mem].fitness;
    copy_gene(population[best_mem], population[POPSIZE], NVARS);
  }
  else
  {
//    for ( i = 0; i < NVARS; i++ )
...
//    population[worst_mem].fitness = population[POPSIZE].fitness;
    copy_gene(population[POPSIZE], population[worst_mem], NVARS);
  } 
...
//    //population[POPSIZE].gene[i] = population[cur_best].gene[i];
    copy_gene(population[cur_best], population[POPSIZE], NVARS);
//  }
...
//
  ...
  { 
    ...
    if ( p < population[0].cfitness )
    {
      ...
      copy_gene(population[0], newpopulation[i], NVARS);
    }
    else
    {
      for ( j = 0; j < POPSIZE; j++ )
      { 
        if ( population[j].cfitness <= p && p < population[j+1].cfitness )
        {
          ...
          copy_gene(population[j+1], newpopulation[i], NVARS);
        }
      }
    }
  }
...
//
  for ( i = 0; i < POPSIZE; i++ )
  {
    ...
    copy_gene(newpopulation[i], population[i], NVARS);
  }
